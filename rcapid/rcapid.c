...
static unsigned char new_controller;
...
rcv_message(struct capi_message *msg) {
	...
	if(read(0, buf, 2) == 2) {
		...
		while(((alen = read(0, &buf[rlen], len)) < len) && (time(NULL) < (t + 3))) { /* timeout 3 sec */
			if (alen > 0) {
				len -= alen;
				rlen += alen;
			}
...
hdl_RCAPI_AUTH_USER_REQ(struct capi_message *msg) 
{
	...
	w2 = get_word(&p);
	b1 = get_byte(&p);
	log(5, "w1:0x%04x w2:0x%04x b1:0x%02x\n", w1,w2,b1);
...
hdl_raw(struct capi_message *msg) {
	...
	if (new_controller) { /* patch new controller */
		p = msg->raw.data + 8;
		controller = *p;
		if ((controller & 0x7f) != 1) {
			log(4, "Wrong controller %d\n", (controller & 0x7f));
			return(1);
		}
		controller &= 0x80;
		*p = (controller | new_controller);
		log(5, "Patch controller 1 -> %d\n", new_controller);
	}
...
int main(int argc, char *argv []) {
	...
	while((c = getopt(argc, argv, "l:c:")) != EOF) {
		switch(c) {
			...
			case 'c':	new_controller = atoi(optarg); break;
		}
	}
	...
	while(! feof(stdin)) {
		...
		if(capi_fd >= 0 && FD_ISSET(capi_fd, &ifd)) {
			...
			if (new_controller) { /* patch to controller 1 */
				buf[10] &= 0x80;
				buf[10] |= 0x01;
				log(5, "Patch controller -> 1\n");
			}
