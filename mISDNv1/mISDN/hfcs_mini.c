...
static const char hfcsmini_rev[] = "Revision: 1.11";
...
hfcsmini_ph_command(channel_t * dch, u_char command)
{
	...
	case HFC_L1_ACTIVATE_TE:
		...
		write_hfcsmini(hw, R_ST_WR_STA, 0x14);
		udelay(125); /* to be sure INFO1 signals are sent */
		write_hfcsmini(hw, R_ST_WR_STA, 0x04);
		break;

	case HFC_L1_FORCE_DEACTIVATE_TE:
		write_hfcsmini(hw, R_ST_WR_STA, 0x13);
		udelay(7); /* wait at least 5,21 us */
		write_hfcsmini(hw, R_ST_WR_STA, 0x03);
		break;
	...
	case HFC_L1_ACTIVATE_NT:
		...
		write_hfcsmini(hw, R_ST_WR_STA, (M_SET_G2_G3 | 0x20));
		break;
	...
	case HFC_L1_DEACTIVATE_NT:
		...
		write_hfcsmini(hw, R_ST_WR_STA, 0x40);
		break;
...
handle_dmsg(channel_t *dch, struct sk_buff *skb)
{
	...
	if (hh->prim == (PH_SIGNAL | REQUEST)) {
		...
		if (hh->dinfo == HW_RESET) {
			...
			skb_trim(skb, 0);
			return (mISDN_queueup_newhead(&dch->inst,
			    0, PH_CONTROL | INDICATION, HW_POWERUP, skb));
...
hfcsmini_manager(void *data, u_int prim, void *arg)
{
	...
	case MGR_SETSTACK | INDICATION:
		if ((channel != 2) && (inst->pid.global == 2)) {
			if ((skb = create_link_skb(PH_ACTIVATE |
				REQUEST, 0, 0, NULL, 0))) {
				if (hfcsmini_l2l1(inst, skb))
					dev_kfree_skb(skb);
			}
...
receive_buffer:
	...
	if (rcnt > 0) {
		if (!ch->rx_skb) {
			ch->rx_skb = alloc_stack_skb(ch->maxlen + 3,
			    ch->up_headerlen);
			if (!ch->rx_skb) {
...
hfcsmini_bh_handler(unsigned long ul_hw)
{
	...
	/* Timer Int */
	if (GET_V_TI_IRQ(hw->misc_irq)) {
		hw->misc_irq &= (__u8)(~M_TI_IRQ);
		
		/* add Fifo-Fill info into int_s1 bitfield */
		hw->fifo_irq |= ((read_hfcsmini(hw, R_FILL) ^
		    FIFO_MASK_TX) & hw->fifomask);
		/* Handle TX Fifos */
		for (i = 0; i < hw->max_fifo; i++) {
			if ((1 << (i * 2)) & (hw->fifo_irq)) {
				hw->fifo_irq &= ~(1 << (i * 2));
	...
	/* Handle RX Fifos */
	for (i = 0; i < hw->max_fifo; i++) {
		if ((1 << (i * 2 + 1)) & (hw->fifo_irq)) {
			hw->fifo_irq &= ~(1 << (i * 2 + 1));
			hfcsmini_read_fifo(hw, i);
		}
	}
	/* state machine IRQ */
	if (GET_V_ST_IRQ(hw->misc_irq)) {
		hw->misc_irq &= (__u8)(~M_ST_IRQ);

		state = read_hfcsmini(hw, R_ST_RD_STA);
		if (GET_V_ST_STA(state) != hw->chan[2].state) {
			hw->chan[2].state = GET_V_ST_STA(state);
			s0_new_state(&hw->chan[2]);
		}
	}
...
#endif
{
	...
	if (!(GET_V_IRQ_EN(hw->misc_irqmsk))) {
		if (!(hw->testirq))
			printk(KERN_INFO
	...
	fifo_irq = read_hfcsmini_irq(hw, R_FIFO_IRQ) & hw->fifo_irqmsk;
	misc_irq = read_hfcsmini_irq(hw, R_MISC_IRQ) & hw->misc_irqmsk;
	...
	hw->fifo_irq |= fifo_irq;
	hw->misc_irq |= misc_irq;
 ...
 */
void
setup_fifo(hfcsmini_hw * hw, int fifo, __u8 hdlcreg,
    __u8 con_reg, __u8 irq_enable, __u8 enable)
{
	...
	write_hfcsmini(hw, R_FIFO_IRQMSK, hw->fifo_irqmsk);
...
setup_st(hfcsmini_hw * hw, __u8 bc, __u8 enable)
{
	...
	if (bc) {
		SET_V_B2_EN(hw->st_ctrl0, (enable?1:0));
		SET_V_B2_RX_EN(hw->st_ctrl2, (enable?1:0));
	} else {
		SET_V_B1_EN(hw->st_ctrl0, (enable?1:0));
		SET_V_B1_RX_EN(hw->st_ctrl2, (enable?1:0));
	}

	write_hfcsmini(hw, R_ST_CTRL0, hw->st_ctrl0);
	write_hfcsmini(hw, R_ST_CTRL2, hw->st_ctrl2);

	if (debug & DEBUG_HFC_MODE) {
		printk(KERN_INFO
		    "%s %s: bc(%i) %s, R_ST_CTRL0(0x%02x) R_ST_CTRL2(0x%02x)\n",
		    hw->card_name, __FUNCTION__, bc, enable?"enable":"disable",
		    hw->st_ctrl0, hw->st_ctrl2);
	}
}
...
init_mISDN_channels(hfcsmini_hw * hw)
{
	...
	hw->chan[ch].inst.class_dev.parent = &hw->pdev->dev;
#else
	hw->chan[ch].inst.class_dev.dev = &hw->pdev->dev;
#endif
	mISDN_init_instance(&hw->chan[ch].inst, &hw_mISDNObj, hw,
	    hfcsmini_l2l1);

	hw->chan[ch].inst.pid.layermask = ISDN_LAYER(0);
	sprintf(hw->chan[ch].inst.name, "%s", hw->card_name);
	err = mISDN_initchannel(&hw->chan[ch], MSK_INIT_DCHANNEL,
	    MAX_DFRAME_LEN_L1);
	if (err)
		goto free_channels;
	hw->chan[ch].hw = hw;
	...
	for (b = 0; b < 2; b++) {
		...
		mISDN_init_instance(&hw->chan[b].inst, &hw_mISDNObj, hw,
		    hfcsmini_l2l1);
		hw->chan[b].inst.pid.layermask = ISDN_LAYER(0);
		hw->chan[b].inst.hwlock = &hw->mlock;
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,26)
		hw->chan[b].inst.class_dev.parent = &hw->pdev->dev;
#else
		...
		if (mISDN_initchannel(&hw->chan[b], MSK_INIT_BCHANNEL,
		    MAX_DATA_MEM)) {
			err = -ENOMEM;
			goto free_channels;
		}
	...
	hw->chan[ch].state = 0;
	for (b = 0; b < 2; b++) {
		err = mISDN_ctrl(hw->chan[ch].inst.st, MGR_NEWSTACK | REQUEST,
		    &hw->chan[b].inst);
		if (err) {
			printk(KERN_ERR
...
parse_module_params(hfcsmini_hw * hw)
{
	...
	if (debug & DEBUG_HFC_INIT)
		printk(
		    "%s %s: protocol[%i]=0x%02x, dpid=%d,%s bus-mode:%s %s\n",
...
init_hfcsmini(hfcsmini_hw * hw)
{
	...
	hw->chip_id = read_hfcsmini(hw, R_CHIP_ID);

	if (debug & DEBUG_HFC_INIT)
		printk(KERN_INFO "%s %s ChipID: 0x%x\n", hw->card_name,
		    __FUNCTION__, GET_V_CHIP_ID(hw->chip_id));

	switch (GET_V_CHIP_ID(hw->chip_id)) {
		case CHIP_ID_HFCSMINI:
			hw->max_fifo = 4;
			hw->ti       = 5; /* 8 ms timer interval */
			hw->max_z    = 0x7F;
			break;
		default:
			err = -ENODEV;
	...
	if (hw->portmode & PORT_MODE_TE) {
		/* TE mode */
		hw->st_ctrl0 = 0;
		write_hfcsmini(hw, R_ST_CLK_DLY, 0x0F);
		write_hfcsmini(hw, R_ST_CTRL1, 0);
	} else {
		/* NT mode */
		hw->st_ctrl0 = 4;
		write_hfcsmini(hw, R_ST_CLK_DLY, 0x6C);
	...
	if (hw->portmode & PORT_MODE_BUS_MASTER)
		SET_V_PCM_MD(hw->pcm_md0, 1);
	else
		SET_V_PCM_MD(hw->pcm_md0, 0);

	write_hfcsmini(hw, R_PCM_MD0, hw->pcm_md0);
	write_hfcsmini(hw, R_PCM_MD1, 0);
	write_hfcsmini(hw, R_PCM_MD2, 0);

	/* setup threshold register */
	SET_V_THRES_TX(threshold, (HFCSMINI_TX_THRESHOLD / 8));
	SET_V_THRES_RX(threshold, (HFCSMINI_RX_THRESHOLD / 8));
	write_hfcsmini(hw, R_FIFO_THRES, threshold);
...
disable_interrupts(hfcsmini_hw * hw)
{
	...
	hw->misc_irqmsk = 0;
	write_hfcsmini(hw, R_FIFO_IRQMSK, hw->fifo_irqmsk);
	write_hfcsmini(hw, R_MISC_IRQMSK, hw->misc_irqmsk);
...
enable_interrupts(hfcsmini_hw * hw)
{
	...
	write_hfcsmini(hw, R_TI, hw->ti);

	/* D-RX and D-TX interrupts enable */
	SET_V_FIFO2_TX_IRQMSK(hw->fifo_irqmsk, 1);
	SET_V_FIFO2_RX_IRQMSK(hw->fifo_irqmsk, 1);
	...
	SET_V_ST_IRQMSK(hw->misc_irqmsk, 1); /* enable L1-state change irq */
	SET_V_TI_IRQMSK(hw->misc_irqmsk, 1); /* enable L1-state change irq */
	SET_V_IRQ_EN(hw->misc_irqmsk, 1);    /* enable L1-state change irq */
	write_hfcsmini(hw, R_MISC_IRQMSK, hw->misc_irqmsk);
...
setup_instance(hfcsmini_hw * hw)
{
	...
	if (request_irq(hw->irq, hfcsmini_interrupt, __IRQF_SHARED,
	    "HFC-S mini", hw)) {
...
static struct pci_device_id hfcsmini_ids[] = {
	{
		...
		.subdevice = 0xFFFF,
		.driver_data = (unsigned long) &((hfcsmini_param) {
			0xFF, "HFC-S mini Evaluation Board"}),
		},
	{}
};
...
init_pci_bridge(hfcsmini_hw * hw)
{
	...
	/*
	 * wait until reset sequence finished,
	 * can be redefined after schematic review
	 */
	mdelay(300);
...
hfcsmini_init(void)
{
	...
	if ((err = mISDN_register(&hw_mISDNObj))) {
		printk(KERN_ERR
		    "HFC-S mini: can't register HFC-S mini, error(%d)\n", err);
		goto out;
	}
...
hfcsmini_cleanup(void)
{
	...
	if ((err = mISDN_unregister(&hw_mISDNObj))) {
		printk(KERN_ERR
		    "HFC-S mini: can't unregister HFC-S mini, error(%d)\n",
		    err);
	}
