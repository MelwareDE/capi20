...
static void AddIEArray(struct sk_buff *skb, unsigned char *p)
{
	int len, ielen;
	...
	while (len > 0) {
		if (*p & 0x80) {
			mISDN_AddIE(skb, *p, NULL);
			len--;
			p++;
		} else {
			ielen = 2 + p[1];
			mISDN_AddIE(skb, *p, &p[1]);
			len -= ielen;
			p += ielen;
		}
	}
...
__u16 cmsg2info_req(_cmsg *cmsg, struct sk_buff *skb)
{
	...
	mISDN_AddIE(skb, IE_CALLED_PN, cmsg->CalledPartyNumber);
	AddIEArray(skb, cmsg->Facilitydataarray);
	return 0;
}
...
Send2Application(AppPlci_t *aplci, _cmsg *cmsg)
{
	...
	if (plci) {
		if (test_bit(PLCI_STATE_DISCONNECT_IND, &plci->state)) {
			AppPlciDebug(aplci, CAPI_DBG_APPL_MSG, "%s: dropped due to DISCONNECT_IND: applid(%x) %s msgnr(%d) addr(%08x)",
				__FUNCTION__, cmsg->ApplId, capi_cmd2str(cmsg->Command, cmsg->Subcommand),
...
plci_connect_req(struct FsmInst *fi, int event, void *arg)
{
...
#if 0
	if (!(aplci->contr->layerstatus & 0x1)) {
		Info = CapiProtocolErrorLayer1;
		goto answer;
	}
#endif
	...
	if (aplci->contr->nt) {
		channelid = ContrAllocFreeChannel(aplci->contr, 0x0); /* get any channel */
		if (!channelid)
			goto answer;
		aplci->bch = channelid;
		tmp[0] = IE_CHANNEL_ID;
		tmp[1] = 1;
		tmp[2] = 0x88 + channelid;
		aplci->channel = tmp[2];
...
answer:
	...
	if (aplci->contr->nt) {
		if (!channelid) {
			CMSG_ALLOC(cmsg2);
			AppPlciCmsgHeader(aplci, cmsg2, CAPI_DISCONNECT, CAPI_IND);
			cmsg2->Reason = 0x3480 | CAUSE_NO_CHANNEL;
			if (mISDN_FsmEvent(&aplci->plci_m, EV_PI_DISCONNECT_IND, cmsg2))
				cmsg_free(cmsg2);
		} else {
			AppPlciLinkUp(aplci);
		}
...
plci_connect_resp(struct FsmInst *fi, int event, void *arg)
{
	...
	if (cmsg->Reject == 0) { // accept
		if (AppPlciCheckBprotocol(aplci, cmsg)) {
			AppPlciDebug(aplci, CAPI_DBG_APPL_MSG, "%s: B-protocol not supported",
				__FUNCTION__);
			/* int_error(); */
			memcpy(cause, "\x02\x80\xaf", 3); // IE CAUSE, resource unavailable
			goto resp_error;
		}
...
plci_disconnect_ind(struct FsmInst *fi, int event, void *arg)
{
	...
	if (aplci->contr->nt) {
		ContrFreeChannel(aplci->contr, aplci->bch);
		aplci->bch = 0;
	}
	mISDN_FsmChangeState(fi, ST_PLCI_P_6);
	Send2Application(fi->userdata, arg);
	test_and_set_bit(PLCI_STATE_DISCONNECT_IND, &plci->state);
}
...
plci_cc_disconnect_ind(struct FsmInst *fi, int event, void *arg)
{
	...
	if (fi->state == ST_PLCI_P_2) {
		if (!aplci->contr->nt) {
			AppPlciDebug(aplci, CAPI_DBG_APPL_MSG, "%s: DISC not handled in TEmode",
				__FUNCTION__);
			return;
		}
	}
...
static void
plci_cc_release_indp6(struct FsmInst *fi, int event, void *arg)
{
	AppPlci_t	*aplci = fi->userdata;
...
plci_select_b_protocol_req(struct FsmInst *fi, int event, void *arg)
{
	AppPlci_t	*aplci = fi->userdata;
	_cmsg		*cmsg = arg;
	__u16		Info = CapiMessageNotSupportedInCurrentState;
	int		ret;

	if ((aplci->contr->nt) || (!(fi->state == ST_PLCI_P_2))) {
...
static struct FsmNode fn_plci_list[] =
{
  ...
  {ST_PLCI_P_2,		EV_L3_DISCONNECT_IND,		plci_cc_disconnect_ind},
  ...
  {ST_PLCI_P_2,		EV_AP_SELECT_B_PROTOCOL_REQ,	plci_select_b_protocol_req},
  ...
  {ST_PLCI_P_6,		EV_L3_RELEASE_IND,		plci_cc_release_indp6},
...
AppPlciConstr(AppPlci_t **aplci, Application_t *appl, Plci_t *plci)
{
	...
	test_and_clear_bit(APLCI_STATE_RX_OFF, &apl->state);
	*aplci = apl;
	return(0);
}
...
static void
ph_control_confirm(AppPlci_t *aplci, struct sk_buff *skb)
{
	mISDN_head_t		*hh;

	hh = mISDN_HEAD_P(skb);

	if (hh->dinfo == HW_FEATURES) {
		if (skb->len == sizeof(aplci->features)) {
			memcpy(&aplci->features, skb->data, skb->len);
			AppPlciDebug(aplci, CAPI_DBG_APPL_MSG,
				"%s: hardware features received (id=%d slots=%d banks=%d).",
				__FUNCTION__, aplci->features.pcm_id, aplci->features.pcm_slots,
				aplci->features.pcm_banks);
		} else {
			AppPlciDebug(aplci, CAPI_DBG_APPL_MSG, "%s: hardware features size mismatch.",
				__FUNCTION__);
		}
	}
	dev_kfree_skb(skb);
...
PL_l3l4(mISDNinstance_t *inst, struct sk_buff *skb)
{
	...
	if (hh->prim == (PH_CONTROL | CONFIRM)) {
		ph_control_confirm(aplci, skb);
		return 0;
	}
	if (hh->prim == CAPI_MESSAGE_REQUEST) {
		if (!ncci) {
			capidebug(CAPI_DBG_NCCI_L3, "%s: NCCI invalid for CAPI_MESSAGE_REQUEST", __FUNCTION__);
			/* int_error(); */
			return(-EINVAL);
		}
	...
	if (!ncci) {
		if ((hh->prim != (DL_ESTABLISH | INDICATION)) && (hh->prim != (DL_ESTABLISH | CONFIRM))) {
			if (hh->prim != (DL_RELEASE | CONFIRM)) {
				/* it can happen that NCCI is down and data is still delivered from buffer */
				/* int_error(); */
				capidebug(CAPI_DBG_NCCI_L3, "%s: NCCI invalid", __FUNCTION__);
			}
			return(-ENODEV);
		}
...
AppPlciLinkUp(AppPlci_t *aplci)
{
	...
	if (aplci->link) {
		capidebug(CAPI_DBG_NCCI, "AppPlciLinkUp already active aplci->link(%p)",
			aplci->link);
		return -EBUSY;
	}
	...
	if (retval) {
		printk(KERN_WARNING "%s MGR_ADDSTPARA | REQUEST ret(%d)\n",
			__FUNCTION__, retval);
	}
	...
	test_and_clear_bit(APLCI_STATE_RX_OFF, &aplci->state);
#if 0
	{
		struct sk_buff *skb;
		skb = create_link_skb(PH_CONTROL | REQUEST, HW_FEATURES, 0, NULL, 0);
		if (skb) if (mISDN_queue_down(&aplci->link->inst, 2, skb)) {
			dev_kfree_skb(skb);
		}
	}
#endif
...
AppPlciInfoIndIE(AppPlci_t *aplci, unsigned char ie, __u32 mask, Q931_info_t *qi)
{
	...
	if ((aplci->contr->nt) && (ie == IE_CALLED_PN)) {
		if (iep[0] < 2) {
			/* empty called party number info will not be sent to appl in NT mode */
			return;
		}
	}
	CMSG_ALLOC(cmsg);
	...
	if (aplci->appl->InfoMask & CAPI_INFOMASK_EARLYB3) {
		if ((aplci->contr->nt) || (test_bit(PLCI_STATE_OUTGOING, &aplci->plci->state))) {
			if (((ie == IE_PROGRESS) && (iep[0] == 0x02 && iep[2] == 0x88)) || /* in-band information available */
				((ie == IE_CHANNEL_ID) && (!(aplci->contr->nt)) && (test_bit(PLCI_STATE_OUTGOING, &aplci->plci->state)))) {
				AppPlciLinkUp(aplci);
				if (!test_bit(PLCI_STATE_STACKREADY, &aplci->plci->state)) {
