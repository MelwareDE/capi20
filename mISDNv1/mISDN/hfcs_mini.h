...
#define HDLC_PAR_DCH      (0x02)			/* same for D- and E-channel */
#define CON_HDLC_B_TRANS  (M_HDLC_TRP | 0x08)		/* transparent mode B-channel 32 byte threshold */ 
#define CON_HDLC_B_HDLC   (0x08)			/* HDLC mode b-channel */
#define CON_HDLC_D_HDLC   (0x08 | M_IFF)		/* HDLC mode D-channel, 1 fill mode */
#define CON_HDLC_B_LOOP   (0x08 | M1_DATA_FLOW*6)	/* B-channel loopback mode */
...
typedef struct _hfcmini_hw {
	...
	__u8 chip_id;	/* Chip ID */

	__u8 pcm_md0;	/* PCM config */
	__u8 pcm_md1;	/* PCM config */
	__u8 pcm_md2;	/* PCM config */

	__u8 ti;		/* timer interrupt configuration */

	__u8 fifo_irqmsk;	/* FIFO interrupt mask */ 
	__u8 fifo_irq;	/* FIFO interrupt state */

	__u8 misc_irqmsk;	/* MISC interrupt mask */
	__u8 misc_irq;	/* MISC interrupt state */
