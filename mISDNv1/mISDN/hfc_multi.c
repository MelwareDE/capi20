 ...
 * poll:
	...
	reduce cpu lorerunsure, don't mess with it!
...
static const PCI_ENTRY id_list[] =
{
	...
	{CCAG_VID, CCAG_VID, HFC4S_ID, 0xB620, VENDOR_CCD,
	 "HFC-4S", 0, 4, 1, 2, 0},
	{CCAG_VID, CCAG_VID, HFC4S_ID, 0xB558, VENDOR_CCD,    //ucpbx 0xB550->0xB558
	...
	{CCAG_VID, CCAG_VID, HFC8S_ID, 0xB552, VENDOR_CCD,
	 "HFC-8S", 0, 8, 1, 0, 0},
	{CCAG_VID, CCAG_VID, HFC8S_ID, 0xB55B, VENDOR_CCD,
	 "HFC-8S-Junghans", 0, 8, 1, 0, 0},
	...
	{CCAG_VID, CCAG_VID, HFCE1_ID, 0xB564, VENDOR_CCD,
	 "HFC-E1 Beronet Card (Dual)", 1, 1, 0, 1, 0}, /* E1 only supports single clock */
	{CCAG_VID, CCAG_VID, HFCE1_ID, 0xB553, VENDOR_CCD,
	 "HFC-E1 Junghanns Card", 1, 1, 0, 1, 0}, /* E1 only supports single clock */
	...
	{CCAG_VID, CCAG_VID, HFC4S_ID, 0xE888, VENDOR_CCD,
...
module_param(pwm0, uint, S_IRUGO | S_IWUSR);
module_param(pwm_md, uint, S_IRUGO | S_IWUSR);
	...
	if (test_bit(FLG_HDLC, &chan->Flags)) {
		...
		if (f1 != f2) {
			...
			if (chan->rx_skb->data[chan->rx_skb->len - 1]) {
				printk(KERN_WARNING "%s: CRC-error\n", __FUNCTION__);
...
handle_timer_irq(hfc_multi_t *hc)
{
	...
	while(ch < 32) {
		...
		if (chan && hc->created[hc->chan[ch].port]) {
			...
			if (hc->chan[ch].slot_rx < 0) {
				/* don't try to read FIFO if PCM is used */
...
//	u_long		flags;
	...
	if (hc->type == 1) {
		...
		hc->chan[16].conf = -1;
		mode_hfcmulti(hc, 16, nt_mode?ISDN_PID_L1_NT_E1:ISDN_PID_L1_TE_E1, -1, -1, -1, -1);
		...
		while (i < 30) {
			...
			mode_hfcmulti(hc, i+1+(i>=15), ISDN_PID_NONE, -1, -1, -1, -1);
			i++;
		}
	} else {
		i = 0;
		while (i < hc->ports) {
			...
			mode_hfcmulti(hc, (i<<2)+2, nt_mode?ISDN_PID_L1_NT_S0:ISDN_PID_L1_TE_S0, -1, -1, -1, -1);
			...
			hc->chan[i<<2].conf = -1;
			mode_hfcmulti(hc, i<<2, ISDN_PID_NONE, -1, -1, -1, -1);
			hc->chan[(i<<2)+1].slot_tx = -1;
			hc->chan[(i<<2)+1].slot_rx = -1;
			hc->chan[(i<<2)+1].conf = -1;
			mode_hfcmulti(hc, (i<<2)+1, ISDN_PID_NONE, -1, -1, -1, -1);
			i++;
		}
	}
	...
	if (pwm0) {
		...
		printk(" Setting PWM0 to 0x%x\n", pwm0);
		printk(" Setting PWM_MD to 0x%x\n", pwm_md);

		HFC_outb(hc, R_PWM_MD, pwm_md);
		HFC_outb(hc, R_PWM0, pwm0);
	}
...
static struct pci_device_id hfmultipci_ids[] __devinitdata = {
	...
	{ CCAG_VID, 0x16B8   , CCAG_VID, 0xB55B, 0, 0, 0 }, //IOB8ST - Junghans 
	...
	{ CCAG_VID, 0x30B1   , CCAG_VID, 0xB553, 0, 0, 0 }, //Junghanns E1
