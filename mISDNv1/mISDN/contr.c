...
ControllerRun(Controller_t *contr)
{
	...
	contrDebug(contr, CAPI_DBG_INFO,
	    "%s:%s GLOBAL(%08X) B1(%08X) B2(%08X) B3(%08X)", __FUNCTION__,
		(contr->nt) ? " NT" : "",
...
{
	Controller_t *contr = NULL, *tcontr;
	Application_t *ap, *tap;
	AppPlci_t *aplci = NULL;
	...
	if (contr) {
		list_for_each_entry_safe(ap, tap, &contr->Applications, head) {
			aplci = getAppPlci4addr(ap, addr);
			if (aplci)
				break;
		}
	}
...
SendMessage(struct capi_ctr *ctrl, struct sk_buff *skb)
{
	...
	unsigned int debugval = CAPI_DBG_CONTR_MSG;
	...
	if ((cmd == CAPI_DATA_B3_REQ) || (cmd == CAPI_DATA_B3_RESP)) {
		debugval = CAPI_DBG_NCCI_L3;
	}
	contrDebug(contr, debugval,
	...
	case CAPI_RESET_B3_RESP:
		...
		if (err) {
			contrDebug(contr, CAPI_DBG_CONTR_MSG, "mISDN_queue_message return(%d)", err);
			err = CAPI_MSGBUSY;
		}
		break;
	// new NCCI
...
ControllerNewPlci(Controller_t *contr, Plci_t  **plci_p, u_int l3id)
{
	...
	*plci_p = plci;
	...
	test_and_clear_bit(PLCI_STATE_DISCONNECT_IND, &plci->state);
...
Controller_function(mISDNinstance_t *inst, struct sk_buff *skb)
{
	...
	case MGR_SHORTSTATUS | INDICATION:
		if (hh->dinfo & SSTATUS_L1) {
			contrDebug(contr, CAPI_DBG_CONTR_INFO, "%s: contr %d L1 status %d",
				__FUNCTION__, contr->addr, (hh->dinfo & 0x1));
			contr->layerstatus = (hh->dinfo & 0x1);
		}
		return ret;
	...
	if (hh->prim == CAPI_MESSAGE_REQUEST) {
		...
		if (!appl) {
			/* 
			 * we ignore this error, which can happen in normal CAPI process
			int_error();
			*/
			return ret;
		}
    ...
    mISDNobject_t *ocapi)
{
	...
	if (pid->protocol[4] == ISDN_PID_L4_CAPI20NET) {
		contr->nt = 1;
	}
...
//		plink->inst.down.stat = IF_NOACTIV;
	...
	list_add_tail(&contr->controllerlist, &controllers);
...
void ContrFreeChannel(Controller_t *contr, int channel)
{
	u_long			flags;

	contrDebug(contr, CAPI_DBG_CONTR_INFO,
		    "%s: free channel %d", __FUNCTION__, channel);

	if (!channel)
		return;

	if ((channel - 1) > sizeof(contr->bchannelmask))
		return;

	spin_lock_irqsave(&contr->list_lock, flags);
	contr->bchannelmask &= ~(1 << (channel - 1));
	spin_unlock_irqrestore(&contr->list_lock, flags);
}

int ContrAllocFreeChannel(Controller_t *contr, unsigned char id)
{
	int channelid = 0;
	int i;
	u_long			flags;
	...
	if ((ch == 0) || (id & 0x04)) /* no channel or d-channel */
		return 0;

	spin_lock_irqsave(&contr->list_lock, flags);

	/* first check for excl */
	if ((ch > 0) && (ch < 3)) {
		if (!((1 << (ch - 1)) & contr->bchannelmask)) {
			contr->bchannelmask |= (1 << (ch - 1));
			channelid = ch;
			goto out;
		} else if (excl) {
			goto out;
	...
	for (i = 0; i < contr->nr_bc; i++) {
		if ((1 << i) & contr->bchannelmask)
			continue;
		channelid = i + 1;
		contr->bchannelmask |= (1 << i);
		break;
	}
...
out:
	...
	contrDebug(contr, CAPI_DBG_CONTR_INFO,
		    "%s: alloc channel %d", __FUNCTION__, channelid);
	return channelid;
}
