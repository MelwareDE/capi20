...
{
	return mISDN_queue_data(&l2->inst,
...
l2down_skb(layer2_t *l2, struct sk_buff *skb) {
	...
	if (ret && l2->debug)
		printk(KERN_DEBUG "l2down_skb TE: ret(%d)\n", ret);
	return ret;
}
...
ReleaseWin(layer2_t *l2)
{
	...
	if ((cnt = freewin(l2)))
		printk(KERN_WARNING
		    "isdnl2 TE freed %d skbuffs in release\n", cnt);
}
...
send_uframe(layer2_t *l2, struct sk_buff *skb, u_char cmd, u_char cr)
{
	...
	if (skb)
		...
		printk(KERN_WARNING "%s TE: can't alloc skbuff\n", __FUNCTION__);
		return;
	}
...
enquiry_cr(layer2_t *l2, u_char typ, u_char cr, u_char pf)
{
	...
	if (!(skb = alloc_skb(i, GFP_ATOMIC))) {
		printk(KERN_WARNING
		    "isdnl2 TE can't alloc sbbuff for enquiry_cr\n");
		return;
	}
	memcpy(skb_put(skb, i), tmp, i);
	enqueue_super(l2, skb);
...
l2_got_iframe(struct FsmInst *fi, int event, void *arg)
{
	...
	if (test_bit(FLG_OWN_BUSY, &l2->flag)) {
		...
		if (l2->vr == ns) {
			...
			if (l2up(l2, DL_DATA | INDICATION,
			    mISDN_HEAD_DINFO(skb), skb))
				dev_kfree_skb(skb);
		} else {
			/* n(s)!=v(r) */
			dev_kfree_skb(skb);
...
l2_pull_iqueue(struct FsmInst *fi, int event, void *arg)
{
	...
	p1 = (p1 + l2->sow) % l2->window;
	if (l2->windowar[p1]) {
		printk(KERN_WARNING "isdnl2 TE try overwrite ack queue entry %d\n",
		    p1);
	...
	if (p1 >= i)
		...
		printk(KERN_WARNING
		    "isdnl2 TE pull_iqueue skb header(%d/%d) too short\n", i, p1);
		oskb = nskb;
		nskb = alloc_skb(oskb->len + i, GFP_ATOMIC);
		if (!nskb) {
			dev_kfree_skb(oskb);
			printk(KERN_WARNING "%s TE: no skb mem\n", __FUNCTION__);
			return;
		}
...
ph_data_indication(layer2_t *l2, mISDN_head_t *hh, struct sk_buff *skb) {
	...
	if (test_bit(FLG_LAPD, &l2->flag)) {
		...
		if ((psapi & 1) || !(ptei & 1)) {
			printk(KERN_WARNING
			    "l2 TE D-channel addr:%x frame wrong EA0/EA1\n",
			    l2->inst.id);
			return ret;
		}
		psapi >>= 2;
	...
	if (c) {
		printk(KERN_WARNING "l2 TE D-channel frame error %c\n", c);
...
l2_function(mISDNinstance_t *inst, struct sk_buff *skb)
{
	...
	if (debug)
		printk(KERN_DEBUG  "%s TE: addr(%08x) prim(%x)\n",
...
tei_l2(layer2_t *l2, struct sk_buff *skb)
{
	...
	case (MDL_ERROR | REQUEST):
		...
		printk(KERN_NOTICE "MDL_ERROR|REQ (tei_l2 TE)\n");
		ret = mISDN_FsmEvent(&l2->l2m, EV_L2_MDL_ERROR, skb);
		break;
...
release_l2(layer2_t *l2)
{
	...
	if (l2->cloneif) {
		printk(KERN_DEBUG "%s TE: cloneif (%p) owner(%p) peer(%p)\n",
		    __FUNCTION__, l2->cloneif,
		    l2->cloneif->owner, l2->cloneif->peer);
		if (l2->cloneif->predecessor)
			l2->cloneif->predecessor->clone = l2->cloneif->clone;
		if (l2->cloneif->clone)
			l2->cloneif->clone->predecessor =
...
new_l2(mISDNstack_t *st, mISDN_pid_t *pid) {
	...
	if (!(nl2 = kzalloc(sizeof(layer2_t), GFP_ATOMIC))) {
		printk(KERN_ERR "kmalloc layer2 TE failed\n");
		return -ENOMEM;
	}
	nl2->debug = debug;
	nl2->next_id = 1;
	...
	case ISDN_PID_L2_LAPD:
		...
		if (pid->protocol[3] & ISDN_PID_L3_DF_CRLEN2) {
			if (debug)
				printk("layer2 TE: Windowsize 7\n");
			nl2->window = 7;
		} else {
			if (debug)
				printk("layer2 TE: Windowsize 1\n");
			nl2->window = 1;
		}
	...
	case ISDN_PID_L2_B_X75SLP:
		...
		break;
	default:
		printk(KERN_ERR "layer2 TE create failed prt %x\n",
			pid->protocol[2]);
		kfree(nl2);
		return -ENOPROTOOPT;
	}
	...
	if (err) {
		printk(KERN_WARNING
		    "mISDN %s TE: MGR_NEWENTITY REQUEST failed err(%d)\n",
		    __FUNCTION__, err);
	}
...
clone_l2(layer2_t *l2, mISDNinstance_t **new_ip) {
	...
	if (err) {
		printk(KERN_ERR "clone l2 TE failed err(%d)\n", err);
		return err;
	}
	nl2->cloneif = nif;
...
l2_manager(void *data, u_int prim, void *arg) {
	...
	if (debug & 0x1000)
		printk(KERN_DEBUG "%s TE: data:%p prim:%x arg:%p\n", __FUNCTION__,
			data, prim, arg);
	if (!data)
		return err;
	spin_lock_irqsave(&isdnl2.lock, flags);
	...
	if (err) {
		if (debug & 0x1)
			printk(KERN_WARNING
			    "l2_manager prim(%x) l2 TE no instance\n", prim);
		return err;
	}
	switch (prim) {
	...
	case MGR_SETIF | INDICATION:
		return mISDN_SetIF(inst, arg, prim,
		    l2from_up, l2from_down, l2l);
	...
	default:
		if (debug & 0x1)
			printk(KERN_WARNING
			    "l2_manager  TE prim %x not handled\n", prim);
		return -EINVAL;
	}
	return 0;
}
...
Isdnl2_cleanup(void)
{
	...
	if ((err = mISDN_unregister(&isdnl2))) {
		printk(KERN_ERR
		    "Can't unregister ISDN layer 2 TE error(%d)\n", err);
	}
	if (!list_empty(&isdnl2.ilist)) {
		printk(KERN_WARNING "mISDNl2 TE l2 list not empty\n");
