 ...
 *
 * Author
 *   Martin Bachem (info@colognechip.com, m.bachem@gmx.de)
 *    - based on HiSax hfc_usb.c driver by Peter Sprenger
...
const char *hfcsusb_rev = "Revision: 1.33 (master), 2007-09-03";

#define MAX_CARDS	8
static int hfcsusb_cnt;
static u_int protocol[MAX_CARDS] = {2,2,2,2,2,2,2,2};
static u_int ptptei[MAX_CARDS] = {0,0,0,0,0,0,0,0};
...
MODULE_PARM(ptptei, MODULE_PARM_T);
	...
	switch (command) {
		...
		case HFC_L1_ACTIVATE_NT:
			if (card->chan[D].state == 3) {
				mISDN_queue_data(&card->chan[D].inst,
				                 FLG_MSG_UP, PH_ACTIVATE | INDICATION,
				                 0, 0, NULL, 0);
			} else {
...
hfcsusb_l2l1(mISDNinstance_t *inst, struct sk_buff *skb)
{
	...
	if ((hh->prim == PH_DATA_REQ) || (hh->prim == DL_DATA_REQ)) {
		...
		if (ret > 0) {
			...
			if (!(chan)) {
				printk (KERN_INFO "HFC-S USB: CRITICAL ERROR! "
				                  "chan is NULL pointer!\n");
				spin_unlock_irqrestore(inst->hwlock, flags);
				return(-EINVAL);
			}

			if (!(chan->tx_skb)) {
				printk (KERN_INFO "HFC-S USB: CRITICAL ERROR! channel_senddata "
				                  "returned %d without chan->tx_skb\n", ret);
				spin_unlock_irqrestore(inst->hwlock, flags);
				return(-EINVAL);
			}
...
#endif
{
	...
	if (fifo->active && !status) {
		...
		for (k = 0; k < num_isoc_packets; ++k)
		{
			...
			if (ch->debug && iso_status && !card->disc_flag) {
				printk(KERN_INFO
...
setup_instance(hfcsusb_t * card)
{
	...
	// PTP Mode uses fixed TEI
	if (protocol[hfcsusb_cnt] & 0x20)
		pid.ptp_tei = ptptei[hfcsusb_cnt];
	...
	/* setup extra options, e.g. Line Loops */
	s_ctrl = 0;
	s_ctrl_r = 0;
	if (protocol[hfcsusb_cnt-1] & 0x80) {
		printk (KERN_INFO "HFC-S USB: card(%d) init B1 loop "
		       "(ST-RX -> XHFC PCM -> ST-TX)\n",
		       hfcsusb_cnt);

		write_usb(card, HFCUSB_B1_SSL, 0x80);
		write_usb(card, HFCUSB_B1_RSL, 0xC0);

		write_usb(card, HFCUSB_FIFO, 0);
		write_usb(card, HFCUSB_HDLC_PAR, 0);
		write_usb(card, HFCUSB_CON_HDLC, 0xC8);

		write_usb(card, HFCUSB_FIFO, 1);
		write_usb(card, HFCUSB_HDLC_PAR, 0);
		write_usb(card, HFCUSB_CON_HDLC, 0xC8);
	...
	if (protocol[hfcsusb_cnt-1] & 0x100) {
		printk (KERN_INFO "HFC-S USB: card(%d) init B2 loop "
		       "(ST-RX -> XHFC PCM -> ST-TX)\n",
		       hfcsusb_cnt);

		write_usb(card, HFCUSB_B2_SSL, 0x81);
		write_usb(card, HFCUSB_B2_RSL, 0xC1);

		write_usb(card, HFCUSB_FIFO, 2);
		write_usb(card, HFCUSB_HDLC_PAR, 0);
		write_usb(card, HFCUSB_CON_HDLC, 0xC8);

		write_usb(card, HFCUSB_FIFO, 3);
		write_usb(card, HFCUSB_HDLC_PAR, 0);
		write_usb(card, HFCUSB_CON_HDLC, 0xC8);
	...
	/*
	if (options[hfcsusb_cnt-1] & 0x0200) {
		printk (KERN_INFO "HFC-S USB: card(%d) init D loop "
		                  "(ST-RX -> XHFC PCM -> ST-TX)\n",
		                  hfcsusb_cnt);
	}
	*/
	if (s_ctrl)
		write_usb(card, HFCUSB_SCTRL, s_ctrl);
	if (s_ctrl_r)
		write_usb(card, HFCUSB_SCTRL_R, s_ctrl_r);
