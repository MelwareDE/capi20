...
static char *STAT_PROC_ENTRY = "mISDN_capi";
static struct proc_dir_entry *stat_procent;
...
static int
stat_read_proc(char *page, char **start, off_t off, int count, int *eof,
    void *data)
{
	Controller_t	*ctrl;
	int len = 0;
	...
	len += sprintf(page+len, "#addr bch nt applid L1\n");
	spin_lock_irqsave(&capi_obj.lock, flags);
	list_for_each_entry(ctrl, &capi_obj.ilist, list) {
		for (bch = 1; bch <= ctrl->nr_bc; bch++) {
			applid = 0;
			if (ctrl->plcis) {
				plci = ctrl->plcis;
				for (i = 0; i < ctrl->maxplci; i++) {
					if (test_bit(PLCI_STATE_ACTIV, &plci->state)) {
						if (plci->nAppl) {
							list_for_each_safe(item, next, &plci->AppPlcis) {
								aplci = (AppPlci_t *)item;
								if ((aplci->link) && ((aplci->channel & 0x7) == bch) && (aplci->appl)) {
									applid = aplci->appl->ApplId;
									break;
								}
							}
						}
			...
			len += sprintf(page+len, "%d %d %d %d %d\n",
				ctrl->addr, bch, ctrl->nt, applid, (ctrl->layerstatus & 0x1));
		}
	}
	spin_unlock_irqrestore(&capi_obj.lock, flags);
...
Capi20Init(void)
{
...
#endif
	capi_obj.name = MName;
	capi_obj.DPROTO.protocol[4] = ISDN_PID_L4_CAPI20 | ISDN_PID_L4_CAPI20NET;
	...
	if ((err = mISDN_register(&capi_obj))) {
		...
		stat_procent = create_proc_entry(STAT_PROC_ENTRY, 0, NULL);
		if (stat_procent) {
			stat_procent->read_proc = stat_read_proc;
		}
	}
	return (err);
}
...
static void Capi20cleanup(void)
{
	...
	if (stat_procent) {
		remove_proc_entry(STAT_PROC_ENTRY, NULL);
	}
